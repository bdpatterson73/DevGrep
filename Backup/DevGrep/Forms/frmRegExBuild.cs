...
namespace DevGrep.Forms
{
	...
	public class frmRegExBuild : System.Windows.Forms.Form
	{
        ...
        private void InitValues()
        {
            ...
            DataSets.dsRegEx re = new dsRegEx() ;
            re.ReadXml(Application.StartupPath + "\\regex.dat");
            foreach (DataRow dr in re.Tables[0].Rows)
            {
                if (dr["EntryType"].ToString().ToUpper()=="FOLDER" &&
                    dr["ParentHash"].ToString().ToUpper()==rootNodeHash)
                {
                    Console.WriteLine("!!!! " + dr["EntryName"].ToString() );
                    Classes.RegExTreeItem reti = new RegExTreeItem() ;
