using System;
using System.IO;
using TokenStream = SmartSearch.Analysis.TokenStream;
using FieldInvertState = SmartSearch.Index.FieldInvertState;

namespace SmartSearch.Documents
{
	/// <summary> Synonymous with <see cref="Field" />.
	/// 
    /// <p/><bold>WARNING</bold>: This interface may change within minor versions, despite SmartSearch's backward compatibility requirements.
	/// This means new methods may be added from version to version.  This change only affects the Fieldable API; other backwards
    /// compatibility promises remain intact. For example, SmartSearch can still
	...
	public interface IFieldable
	{
        ...
        /// document.
	    /// 
	    /// <p/>The boost is multiplied by <see cref="SmartSearch.Documents.Document.Boost" /> of the document
	    /// containing this field.  If a document has multiple fields with the same
	    /// name, all such values are multiplied together.  This product is then
	    /// used to compute the norm factor for the field.  By
	    /// default, in the <see cref="SmartSearch.Search.Similarity.ComputeNorm(String,SmartSearch.Index.FieldInvertState)"/>
	    /// method, the boost value is multiplied
	    /// by the <see cref="SmartSearch.Search.Similarity.LengthNorm(String,int)"/>
	    /// and then rounded by <see cref="SmartSearch.Search.Similarity.EncodeNorm(float)" /> before it is stored in the
        ...
        /// Documents returned from <see cref="SmartSearch.Index.IndexReader.Document(int)" /> and
        /// <see cref="SmartSearch.Search.Searcher.Doc(int)" /> may thus not have the same value present as when
        /// this field was indexed.
        /// 
	    /// </summary>
	    /// <seealso cref="SmartSearch.Documents.Document.Boost">
	    /// </seealso>
	    /// <seealso cref="SmartSearch.Search.Similarity.ComputeNorm(String, FieldInvertState)">
	    /// </seealso>
	    /// <seealso cref="SmartSearch.Search.Similarity.EncodeNorm(float)">
	    /// </seealso>
	    float Boost { get; set; }
	    ...
	    /// vector, available from <see cref="SmartSearch.Index.IndexReader.GetTermFreqVector(int,String)" />.
	    /// These methods do not provide access to the original content of the field,
	    /// only to terms used to index it. If the original content must be
	    /// preserved, use the <c>stored</c> attribute instead.
	    /// 
	    /// </summary>
	    /// <seealso cref="SmartSearch.Index.IndexReader.GetTermFreqVector(int, String)">
	    /// </seealso>
	    ...
	    /// it's values via <see cref="StringValue()" /> or <see cref="GetBinaryValue()" /> is only valid as long as the <see cref="SmartSearch.Index.IndexReader" /> that
	    ...
	    /// <see cref="SmartSearch.Search.PhraseQuery"/> or 
	    /// <see cref="SmartSearch.Search.Spans.SpanQuery"/> 
