...
using DocIdSet = SmartSearch.Search.DocIdSet;
using DocIdSetIterator = SmartSearch.Search.DocIdSetIterator;
...
namespace SmartSearch.Util
{
	...
	/// borrowed from <see cref="SmartSearch.Store.IndexInput" /> and
	/// <see cref="SmartSearch.Store.IndexOutput" />.<p/>
	...
	public class SortedVIntList:DocIdSet
	{
		private class AnonymousClassDocIdSetIterator:DocIdSetIterator
		{
			...
			{
				// See SmartSearch.Store.IndexInput.readVInt()
				sbyte b = Enclosing_Instance.bytes[bytePos++];
				lastInt += (b & SmartSearch.Util.SortedVIntList.VB1);
				for (int s = SmartSearch.Util.SortedVIntList.BIT_SHIFT; (b & ~ SmartSearch.Util.SortedVIntList.VB1) != 0; s += SmartSearch.Util.SortedVIntList.BIT_SHIFT)
				{
					b = Enclosing_Instance.bytes[bytePos++];
					lastInt += ((b & SmartSearch.Util.SortedVIntList.VB1) << s);
				}
			}
		...
		private class SortedVIntListBuilder
		{
			...
			internal virtual void  AddInt(int nextInt)
			{
				...
				// See SmartSearch.Store.IndexOutput.writeVInt()
				while ((diff & ~ SmartSearch.Util.SortedVIntList.VB1) != 0)
				{
					// The high bit of the next byte needs to be set.
					Enclosing_Instance.bytes[Enclosing_Instance.lastBytePos++] = (sbyte) ((diff & SmartSearch.Util.SortedVIntList.VB1) | ~ SmartSearch.Util.SortedVIntList.VB1);
					diff = Number.URShift(diff, SmartSearch.Util.SortedVIntList.BIT_SHIFT);
				}
