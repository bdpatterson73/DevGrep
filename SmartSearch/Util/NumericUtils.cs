...
namespace SmartSearch.Util
{
	...
	public sealed class NumericUtils
	{
		...
		/// <see cref="SmartSearch.Search.BooleanQuery" /> for each call to its
		...
		public abstract class LongRangeBuilder
		{
			...
			{
				AddRange(SmartSearch.Util.NumericUtils.LongToPrefixCoded(min, shift), SmartSearch.Util.NumericUtils.LongToPrefixCoded(max, shift));
			}
		}
		...
		public abstract class IntRangeBuilder
		{
			...
			{
				AddRange(SmartSearch.Util.NumericUtils.IntToPrefixCoded(min, shift), SmartSearch.Util.NumericUtils.IntToPrefixCoded(max, shift));
			}
		}
	}
}