...
using ReaderUtil = SmartSearch.Util.ReaderUtil;
...
namespace SmartSearch.Search
{
	...
	public class MultiSearcher:Searcher
	{
		...
		{
			public AnonymousClassCollector(SmartSearch.Search.Collector collector, int start, MultiSearcher enclosingInstance)
			{
				InitBlock(collector, start, enclosingInstance);
			}
			private void  InitBlock(SmartSearch.Search.Collector collector, int start, MultiSearcher enclosingInstance)
			{
				this.collector = collector;
				this.start = start;
				this.enclosingInstance = enclosingInstance;
			}
			private SmartSearch.Search.Collector collector;
			private int start;
		...
		public /*protected internal*/ override Weight CreateWeight(Query original)
		{
			...
			// step 2
		    ISet<Term> terms = SmartSearch.Support.Compatibility.SetFactory.CreateHashSet<Term>();
