...
using ToStringUtils = SmartSearch.Util.ToStringUtils;
using Explanation = SmartSearch.Search.Explanation;
using Scorer = SmartSearch.Search.Scorer;
using Searcher = SmartSearch.Search.Searcher;
using Similarity = SmartSearch.Search.Similarity;
using Weight = SmartSearch.Search.Weight;
using NearSpansOrdered = SmartSearch.Search.Spans.NearSpansOrdered;
using NearSpansUnordered = SmartSearch.Search.Spans.NearSpansUnordered;
using SpanNearQuery = SmartSearch.Search.Spans.SpanNearQuery;
using SpanQuery = SmartSearch.Search.Spans.SpanQuery;
using SpanScorer = SmartSearch.Search.Spans.SpanScorer;
using SpanWeight = SmartSearch.Search.Spans.SpanWeight;

namespace SmartSearch.Search.Payloads
{
	
	/// <summary> This class is very similar to
	/// <see cref="SmartSearch.Search.Spans.SpanNearQuery" /> except that it factors
	/// in the value of the payloads located at each of the positions where the
	/// <see cref="SmartSearch.Search.Spans.TermSpans" /> occurs.
	/// <p/>
	/// In order to take advantage of this, you must override
	/// <see cref="SmartSearch.Search.Similarity.ScorePayload" />
	/// which returns 1 by default.
	/// <p/>
	/// Payload scores are aggregated using a pluggable <see cref="PayloadFunction" />.
	/// 
	/// </summary>
	/// <seealso cref="SmartSearch.Search.Similarity.ScorePayload">
	/// </seealso>
	[Serializable]
	public class PayloadNearQuery:SpanNearQuery, System.ICloneable
	{
		...
		public class PayloadNearSpanScorer:SpanScorer
		{
			...
			new internal SmartSearch.Search.Spans.Spans spans;
			
			protected internal float payloadScore;
			private int payloadsSeen;
			internal Similarity similarity;
			
			protected internal PayloadNearSpanScorer(PayloadNearQuery enclosingInstance, SmartSearch.Search.Spans.Spans spans, Weight weight, Similarity similarity, byte[] norms):base(spans, weight, similarity, norms)
			{
				InitBlock(enclosingInstance);
				this.spans = spans;
			}
			...
			public virtual void  GetPayloads(SmartSearch.Search.Spans.Spans[] subSpans)
			{
			...
			public /*protected internal*/ override bool SetFreqCurrentDoc()
			{
				...
				SmartSearch.Search.Spans.Spans[] spansArr = new SmartSearch.Search.Spans.Spans[1];
				spansArr[0] = spans;
				payloadScore = 0;
				payloadsSeen = 0;
