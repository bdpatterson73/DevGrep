...
namespace SmartSearch.Search
{
	...
	public class ParallelMultiSearcher : MultiSearcher/*, IDisposable*/ //No need to implement IDisposable like java, nothing to dispose with the TPL
	{
		...
		{
			public AnonymousClassCollector1(SmartSearch.Search.Collector collector, int start, ParallelMultiSearcher enclosingInstance)
			{
				InitBlock(collector, start, enclosingInstance);
			}
			private void  InitBlock(SmartSearch.Search.Collector collector, int start, ParallelMultiSearcher enclosingInstance)
			{
