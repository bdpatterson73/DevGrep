...
using OpenBitSet = SmartSearch.Util.OpenBitSet;
...
namespace SmartSearch.Search
{
	...
	/// <p/>
	/// This filter does not cache.  See the <see cref="SmartSearch.Search.CachingSpanFilter" /> for a wrapper that
	/// caches.
	/// 
	/// 
	/// </summary>
	...
	public class SpanQueryFilter:SpanFilter
	{
		...
		/// <c>query</c>.
		/// </summary>
		/// <param name="query">The <see cref="SmartSearch.Search.Spans.SpanQuery" /> to use as the basis for the Filter.
		/// </param>
		...
		public override SpanFilterResult BitSpans(IndexReader reader)
		{
			...
			SmartSearch.Search.Spans.Spans spans = internalQuery.GetSpans(reader);
