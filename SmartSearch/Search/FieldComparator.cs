...
using ByteParser = SmartSearch.Search.ByteParser;
using DoubleParser = SmartSearch.Search.DoubleParser;
using FloatParser = SmartSearch.Search.FloatParser;
using IntParser = SmartSearch.Search.IntParser;
using LongParser = SmartSearch.Search.LongParser;
using ShortParser = SmartSearch.Search.ShortParser;
using StringIndex = SmartSearch.Search.StringIndex;
...
namespace SmartSearch.Search
{
	...
	/// 
    /// <item> <see cref="P:SmartSearch.Search.FieldComparator.Item(System.Int32)" /> Return the sort value stored in
	...
	public abstract class FieldComparator
	{
	    ...
	    /// <summary>Parses field's values as byte (using <see cref="FieldCache.GetBytes(SmartSearch.Index.IndexReader,string)" />
		...
		public sealed class ByteComparator:FieldComparator
		{
			...
			internal ByteComparator(int numHits, System.String field, SmartSearch.Search.Parser parser)
			{
			...
			{
				currentReaderValues = SmartSearch.Search.FieldCache_Fields.DEFAULT.GetBytes(reader, field, parser);
			}
		...
		/// <summary>Parses field's values as double (using <see cref="FieldCache.GetDoubles(SmartSearch.Index.IndexReader,string)" />
		/// and sorts by ascending value 
		/// </summary>
		public sealed class DoubleComparator:FieldComparator
		{
			...
			internal DoubleComparator(int numHits, System.String field, SmartSearch.Search.Parser parser)
			{
			...
			public override void  SetNextReader(IndexReader reader, int docBase)
			{
				currentReaderValues = SmartSearch.Search.FieldCache_Fields.DEFAULT.GetDoubles(reader, field, parser);
			}
		...
		/// <summary>Parses field's values as float (using <see cref="FieldCache.GetFloats(SmartSearch.Index.IndexReader,string)" />
		/// and sorts by ascending value 
		/// </summary>
		public sealed class FloatComparator:FieldComparator
		{
			...
			internal FloatComparator(int numHits, System.String field, SmartSearch.Search.Parser parser)
			{
			...
			public override void  SetNextReader(IndexReader reader, int docBase)
			{
				currentReaderValues = SmartSearch.Search.FieldCache_Fields.DEFAULT.GetFloats(reader, field, parser);
			}
		...
		/// <summary>Parses field's values as int (using <see cref="FieldCache.GetInts(SmartSearch.Index.IndexReader,string)" />
		/// and sorts by ascending value 
		/// </summary>
		public sealed class IntComparator:FieldComparator
		{
			...
			internal IntComparator(int numHits, System.String field, SmartSearch.Search.Parser parser)
			{
			...
			public override void  SetNextReader(IndexReader reader, int docBase)
			{
				currentReaderValues = SmartSearch.Search.FieldCache_Fields.DEFAULT.GetInts(reader, field, parser);
			}
		...
		/// <summary>Parses field's values as long (using <see cref="FieldCache.GetLongs(SmartSearch.Index.IndexReader,string)" />
		/// and sorts by ascending value 
		/// </summary>
		public sealed class LongComparator:FieldComparator
		{
			...
			internal LongComparator(int numHits, System.String field, SmartSearch.Search.Parser parser)
			{
			...
			public override void  SetNextReader(IndexReader reader, int docBase)
			{
				currentReaderValues = SmartSearch.Search.FieldCache_Fields.DEFAULT.GetLongs(reader, field, parser);
			}
		...
		public sealed class ShortComparator:FieldComparator
		{
			...
			internal ShortComparator(int numHits, System.String field, SmartSearch.Search.Parser parser)
			{
			...
			public override void  SetNextReader(IndexReader reader, int docBase)
			{
				currentReaderValues = SmartSearch.Search.FieldCache_Fields.DEFAULT.GetShorts(reader, field, parser);
			}
		...
		public sealed class StringComparatorLocale:FieldComparator
		{
			...
			public override void  SetNextReader(IndexReader reader, int docBase)
			{
				currentReaderValues = SmartSearch.Search.FieldCache_Fields.DEFAULT.GetStrings(reader, field);
			}
		...
		public sealed class StringOrdValComparator:FieldComparator
		{
			...
			public override void  SetNextReader(IndexReader reader, int docBase)
			{
				StringIndex currentReaderValues = SmartSearch.Search.FieldCache_Fields.DEFAULT.GetStringIndex(reader, field);
				currentReaderGen++;
