...
using Analyzer = SmartSearch.Analysis.Analyzer;
using Document = SmartSearch.Documents.Document;
using IndexingChain = SmartSearch.Index.DocumentsWriter.IndexingChain;
using AlreadyClosedException = SmartSearch.Store.AlreadyClosedException;
using BufferedIndexInput = SmartSearch.Store.BufferedIndexInput;
using Directory = SmartSearch.Store.Directory;
using Lock = SmartSearch.Store.Lock;
using LockObtainFailedException = SmartSearch.Store.LockObtainFailedException;
using Constants = SmartSearch.Util.Constants;
using Query = SmartSearch.Search.Query;
...
namespace SmartSearch.Index
{
    ...
    */
    public class IndexWriter : System.IDisposable
	{
        ...
        private ISet<SegmentInfo> segmentsToOptimize = SmartSearch.Support.Compatibility.SetFactory.CreateHashSet<SegmentInfo>(); // used by optimize to note those needing optimization
		...
		private ISet<MergePolicy.OneMerge> runningMerges = SmartSearch.Support.Compatibility.SetFactory.CreateHashSet<MergePolicy.OneMerge>();
		...
		/// using <see cref="IndexReader.Open(SmartSearch.Store.Directory, bool)" /> to open a new reader. But the turarnound
		...
		/// <param name="deletionPolicy">see <a href="#deletionPolicy">above</a>
		/// </param>
		/// <param name="mfl"><see cref="SmartSearch.Index.IndexWriter.MaxFieldLength" />, whether or not to limit field lengths.  Value is in number of terms/tokens
		/// </param>
		/// <throws>  CorruptIndexException if the index is corrupt </throws>
		/// <throws>  LockObtainFailedException if another writer </throws>
		/// <summary>  has this index open (<c>write.lock</c> could not
		...
		/// <param name="deletionPolicy">see <a href="#deletionPolicy">above</a>
		/// </param>
		/// <param name="mfl">whether or not to limit field lengths, value is in number of terms/tokens.  See <see cref="SmartSearch.Index.IndexWriter.MaxFieldLength" />.
		/// </param>
		...
		public virtual void  Optimize(int maxNumSegments, bool doWait)
		{
			...
			lock (this)
			{
                ...
                segmentsToOptimize = SmartSearch.Support.Compatibility.SetFactory.CreateHashSet<SegmentInfo>();
		...
		* addIndexesNoOptimize(). */
        ...
        private void SetDiagnostics(SegmentInfo info, System.String source, IDictionary<string, string> details)
		{
			...
			diagnostics["SmartSearch.version"] = Constants.LUCENE_VERSION;
		...
		public sealed class MaxFieldLength
		{
			...
			{
				LIMITED = new MaxFieldLength("LIMITED", SmartSearch.Index.IndexWriter.DEFAULT_MAX_FIELD_LENGTH);
			}
		}
