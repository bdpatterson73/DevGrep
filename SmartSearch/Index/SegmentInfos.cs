...
using ChecksumIndexInput = SmartSearch.Store.ChecksumIndexInput;
using ChecksumIndexOutput = SmartSearch.Store.ChecksumIndexOutput;
using Directory = SmartSearch.Store.Directory;
using IndexInput = SmartSearch.Store.IndexInput;
using IndexOutput = SmartSearch.Store.IndexOutput;
using NoSuchDirectoryException = SmartSearch.Store.NoSuchDirectoryException;
...
namespace SmartSearch.Index
{
	...
	public sealed class SegmentInfos : List<SegmentInfo>, ICloneable
	{
		private class AnonymousClassFindSegmentsFile:FindSegmentsFile
		{
			...
			internal AnonymousClassFindSegmentsFile(SegmentInfos enclosingInstance, SmartSearch.Store.Directory Param1):base(Param1)
			{
			...
			public System.Object Run(IndexCommit commit)
			{
				...
				while (true)
				{
					...
					if (0 == method)
					{
						...
						if (files != null)
							genA = SmartSearch.Index.SegmentInfos.GetCurrentSegmentGeneration(files);
						
						SmartSearch.Index.SegmentInfos.Message("directory listing genA=" + genA);
						...
						long genB = - 1;
						for (int i = 0; i < SmartSearch.Index.SegmentInfos.defaultGenFileRetryCount; i++)
						{
							...
							{
								SmartSearch.Index.SegmentInfos.Message("segments.gen open: FileNotFoundException " + e);
								break;
							}
							catch (System.IO.IOException e)
							{
								SmartSearch.Index.SegmentInfos.Message("segments.gen open: IOException " + e);
							}
							...
							if (genInput != null)
							{
								try
								{
									...
									if (version == SmartSearch.Index.SegmentInfos.FORMAT_LOCKLESS)
									{
										long gen0 = genInput.ReadLong();
										long gen1 = genInput.ReadLong();
										SmartSearch.Index.SegmentInfos.Message("fallback check: " + gen0 + "; " + gen1);
										if (gen0 == gen1)
										{
                            ...
                            System.Threading.Thread.Sleep(new TimeSpan((System.Int64) 10000 * SmartSearch.Index.SegmentInfos.defaultGenFileRetryPauseMsec));
							
							
						}
						
						SmartSearch.Index.SegmentInfos.Message(IndexFileNames.SEGMENTS_GEN + " check: genB=" + genB);
					...
					if (1 == method || (0 == method && lastGen == gen && retry))
					{
						...
						if (genLookaheadCount < SmartSearch.Index.SegmentInfos.defaultGenLookaheadCount)
						{
							gen++;
							genLookaheadCount++;
							SmartSearch.Index.SegmentInfos.Message("look ahead increment gen to " + gen);
						}
					}
					...
					try
					{
						...
						SmartSearch.Index.SegmentInfos.Message("success on " + segmentFileName);
					...
					catch (System.IO.IOException err)
					{
						...
						SmartSearch.Index.SegmentInfos.Message("primary Exception on '" + segmentFileName + "': " + err + "'; will retry: retry=" + retry + "; gen = " + gen);
						...
						if (!retry && gen > 1)
						{
							...
							if (prevExists)
							{
								SmartSearch.Index.SegmentInfos.Message("fallback to prior segment file '" + prevSegmentFileName + "'");
								try
								{
									System.Object v = DoBody(prevSegmentFileName);
									if (exc != null)
									{
										SmartSearch.Index.SegmentInfos.Message("success on fallback " + prevSegmentFileName);
									}
									return v;
								}
								catch (System.IO.IOException err2)
								{
									SmartSearch.Index.SegmentInfos.Message("secondary Exception on '" + prevSegmentFileName + "': " + err2 + "'; will retry");
								}
							}
						}
					}
        ...
        #region SmartSearch (Equals & GetHashCode )
        /// <summary>
