	...
	public class ConcurrentMergeScheduler:MergeScheduler
	{
		...
		protected internal virtual MergeThread GetMergeThread(IndexWriter writer, MergePolicy.OneMerge merge)
		{
			lock (this)
			{
				...
				thread.Name = "SmartSearch Merge Thread #" + mergeThreadCount++;
				return thread;
			}
		}
				...
				catch (System.Exception exc)
				{
					...
					if (!(exc is MergePolicy.MergeAbortedException))
					{
						if (!Enclosing_Instance.suppressExceptions)
						{
							...
							// testing.
							SmartSearch.Index.ConcurrentMergeScheduler.anyExceptions = true;
		...
		public static bool AnyUnhandledExceptions()
		{
			if (allInstances == null)
			{
                throw new System.SystemException("setTestMode() was not called; often this is because your test case's setUp method fails to call super.setUp in SmartSearch");
			}
			lock (allInstances)
			{
