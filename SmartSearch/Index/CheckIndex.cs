...
using AbstractField = SmartSearch.Documents.AbstractField;
using Document = SmartSearch.Documents.Document;
using Directory = SmartSearch.Store.Directory;
using FSDirectory = SmartSearch.Store.FSDirectory;
	...
	public class CheckIndex
	{
		...
		public virtual Status CheckIndex_Renamed_Method(List<string> onlySegments)
		{
			...
			if (format == SegmentInfos.FORMAT)
                sFormat = "FORMAT [SmartSearch Pre-2.1]";
			if (format == SegmentInfos.FORMAT_LOCKLESS)
                sFormat = "FORMAT_LOCKLESS [SmartSearch 2.1]";
			else if (format == SegmentInfos.FORMAT_SINGLE_NORM_FILE)
                sFormat = "FORMAT_SINGLE_NORM_FILE [SmartSearch 2.2]";
			else if (format == SegmentInfos.FORMAT_SHARED_DOC_STORE)
                sFormat = "FORMAT_SHARED_DOC_STORE [SmartSearch 2.3]";
			else
			{
				if (format == SegmentInfos.FORMAT_CHECKSUM)
                    sFormat = "FORMAT_CHECKSUM [SmartSearch 2.4]";
				else if (format == SegmentInfos.FORMAT_DEL_COUNT)
                    sFormat = "FORMAT_DEL_COUNT [SmartSearch 2.4]";
				else if (format == SegmentInfos.FORMAT_HAS_PROX)
                    sFormat = "FORMAT_HAS_PROX [SmartSearch 2.4]";
				else if (format == SegmentInfos.FORMAT_USER_DATA)
                    sFormat = "FORMAT_USER_DATA [SmartSearch 2.9]";
				else if (format == SegmentInfos.FORMAT_DIAGNOSTICS)
                    sFormat = "FORMAT_DIAGNOSTICS [SmartSearch 2.9]";
				else if (format < SegmentInfos.CURRENT_FORMAT)
				{
                    sFormat = "int=" + format + " [newer version of SmartSearch than this tool]";
					skip = true;
				}
				else
				{
                    sFormat = format + " [SmartSearch 1.3 or prior]";
				}
			}
			...
			if (skip)
			{
                Msg("\nERROR: this index appears to be created by a newer version of SmartSearch than this tool was compiled on; please re-compile this tool on the matching version of SmartSearch; exiting");
		...
		/// Run it like this:
        /// <code>
		/// java -ea:SmartSearch... SmartSearch.Index.CheckIndex pathToIndex [-fix] [-segment X] [-segment Y]
        /// </code>
		/// <list type="bullet">
		...
		public static void  Main(System.String[] args)
		{
			...
			if (indexPath == null)
			{
				...
				System.Console.Out.WriteLine("\nUsage: java SmartSearch.Index.CheckIndex pathToIndex [-fix] [-segment X] [-segment Y]\n" + "\n" + "  -fix: actually write a new segments_N file, removing any problematic segments\n" + "  -segment X: only check the specified segments.  This can be specified multiple\n" + "              times, to check more than one segment, eg '-segment _2 -segment _a'.\n" + "              You can't use this with the -fix option\n" + "\n" + "**WARNING**: -fix should only be used on an emergency basis as it will cause\n" + "documents (perhaps many) to be permanently removed from the index.  Always make\n" + "a backup copy of your index before running this!  Do not run this tool on an index\n" + "that is actively being written to.  You have been warned!\n" + "\n" + "Run without -fix, this tool will open the index, report version information\n" + "and report any exceptions it hits and what action it would take if -fix were\n" + "specified.  With -fix, this tool will remove any segments that have issues and\n" + "write a new segments_N file.  This means all documents contained in the affected\n" + "segments will be removed.\n" + "\n" + "This tool exits with exit code 1 if the index cannot be opened or has any\n" + "corruption, else 0.\n");
				System.Environment.Exit(1);
			}
			
			if (!AssertsOn())
				System.Console.Out.WriteLine("\nNOTE: testing will be more thorough if you run java with '-ea:SmartSearch...', so assertions are enabled");
