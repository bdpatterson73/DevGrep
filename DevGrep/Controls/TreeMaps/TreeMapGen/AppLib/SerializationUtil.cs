...
namespace DevGrep.Controls.TreeMaps.TreeMapGen.AppLib
{
    public class SerializationUtil
    {
        ...
        public static void SerializeStringAttributes(XmlTextWriter oXmlTextWriter, params string[] asNameValuePairs)
        {
            ...
            if (num%2 != 0)
            {
                throw new ApplicationException(
                    "SerializationUtil.SerializeStringAttributes: asNameValuePairs must contain an even number of elements.");
            }
            for (int i = 0; i < num; i += 2)
            {
                string text = asNameValuePairs[i];
                string text2 = asNameValuePairs[i + 1];
                if (StringUtil.IsEmpty(text))
                {
                    throw new ApplicationException(
                        string.Format(
                            "SerializationUtil.SerializeStringAttributes: asNameValuePairs[{0}] is empty or null.", i));
                }
                oXmlTextWriter.WriteAttributeString(text, text2);
            }
        }

        public static string DeserializeRequiredStringAttribute(XmlTextReader oXmlTextReader, string sElementName,
                                                                string sAttributeName)
        {
            ...
            if (attribute == null)
            {
                throw new ApplicationException(
                    string.Format(
                        "SerializationUtil.DeserializeRequiredStringAttribute: A {0} XML element is missing a required {1} attribute.",
                        sElementName, sAttributeName));
            }
            return attribute;
        }

        public static int DeserializeRequiredInt32Attribute(XmlTextReader oXmlTextReader, string sElementName,
                                                            string sAttributeName)
        {
            Debug.Assert(oXmlTextReader != null);
            Debug.Assert(oXmlTextReader.NodeType == XmlNodeType.Element);
            ...
            StringUtil.AssertNotEmpty(sElementName);
            StringUtil.AssertNotEmpty(sAttributeName);
            string text = DeserializeRequiredStringAttribute(oXmlTextReader, sElementName, sAttributeName);
            ...
            catch (FormatException ex)
            {
                throw new ApplicationException(
                    string.Format(
                        "SerializationUtil.DeserializeRequiredInt32Attribute: A {0} XML element has a {1} attribute that must be an Int32 but is not in Int32 format.",
                        sElementName, sAttributeName), ex);
            }
            return result;
        }

        public static float DeserializeRequiredSingleAttribute(XmlTextReader oXmlTextReader, string sElementName,
                                                               string sAttributeName)
        {
            Debug.Assert(oXmlTextReader != null);
            Debug.Assert(oXmlTextReader.NodeType == XmlNodeType.Element);
            ...
            catch (FormatException ex)
            {
                throw new ApplicationException(
                    string.Format(
                        "SerializationUtil.DeserializeRequiredSingleAttribute: A {0} XML element has a {1} attribute that must be a Single but is not in Single format.",
                        sElementName, sAttributeName), ex);
            }
            return result;
        }
