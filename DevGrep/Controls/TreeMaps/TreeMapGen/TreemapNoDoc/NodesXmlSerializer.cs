...
namespace DevGrep.Controls.TreeMaps.TreeMapGen.TreemapNoDoc
{
    internal class NodesXmlSerializer
    {
        ...
        public Nodes DeserializeFromString(string sNodesXml, ITreemapComponent oTreemapComponent)
        {
            ...
            Debug.Assert(oTreemapComponent != null);
            AssertValid();
            var stringReader = new StringReader(sNodesXml);
            ...
            try
            {
                ...
                if ((!isEmptyElement && xmlTextReader.NodeType != XmlNodeType.EndElement) ||
                    xmlTextReader.Name != "Nodes")
                {
                    throw new ApplicationException(
                        string.Format("The top-level {0} XML element is missing a closing element.", "Nodes"));
                }
            }
            catch (Exception ex)
            {
                throw new ApplicationException(
                    "ITreemapComponent.NodesXml: The XML string is not in the expected format.", ex);
            }
            finally
            {
                xmlTextReader.Close();
        ...
        protected void SerializeNodes(Nodes oNodes, XmlTextWriter oXmlTextWriter)
        {
            ...
            {
                SerializeNode(current, oXmlTextWriter);
            }
        ...
        protected void SerializeNode(Node oNode, XmlTextWriter oXmlTextWriter)
        {
            ...
            SerializationUtil.SerializeStringAttributes(oXmlTextWriter, new[]
                {
                    "Text",
                    oNode.Text,
                    "SizeMetric",
        ...
        protected void DeserializeNodes(XmlTextReader oXmlTextReader, ITreemapComponent oTreemapComponent, Nodes oNodes)
        {
            ...
            Debug.Assert(oNodes != null);
            AssertValid();
            oNodes.EmptySpace.SizeMetric = SerializationUtil.DeserializeRequiredSingleAttribute(oXmlTextReader, "Nodes",
                                                                                                "EmptySizeMetric");
                ...
                while (oXmlTextReader.Read())
                {
                    ...
                    if (nodeType != XmlNodeType.Element)
                    {
                        if (nodeType != XmlNodeType.EndElement)
                        {
                            throw new ApplicationException(
                                string.Format(
                                    "A {0} XML element has an unexpected child node of type {1}.  Only {2} child nodes are allowed.",
                                    "Nodes", oXmlTextReader.NodeType, "Node"));
                        }
                        if (oXmlTextReader.Name != "Nodes")
                        {
                            throw new ApplicationException(
                                string.Format("A {0} XML element is missing a closing element.", "Nodes"));
                        }
                        break;
                    }
                    else
                    {
                        if (oXmlTextReader.Name != "Node")
                        {
                            throw new ApplicationException(
                                string.Format(
                                    "A {0} XML element has an unexpected child node named {1}.  Only {2} child nodes are allowed.",
                                    "Nodes", oXmlTextReader.Name, "Node"));
                        }
                        ...
                        {
                            throw new ApplicationException(
                                string.Format("A {0} XML element is missing a closing element.", "Node"));
                        }
                    }
                }
            }
        ...
        protected Node DeserializeNode(XmlTextReader oXmlTextReader, ITreemapComponent oTreemapComponent)
        {
            ...
            float colorMetric = SerializationUtil.DeserializeRequiredSingleAttribute(oXmlTextReader, "Node",
                                                                                     "ColorMetric");
            Color absoluteColor =
                Color.FromArgb(SerializationUtil.DeserializeRequiredInt32Attribute(oXmlTextReader, "Node",
                                                                                   "AbsoluteColor"));
            ...
            var node = new Node(text, sizeMetric, colorMetric, attribute, toolTip);
            ...
            if (!oXmlTextReader.Read() || oXmlTextReader.NodeType != XmlNodeType.Element ||
                oXmlTextReader.Name != "Nodes")
            {
                throw new ApplicationException(string.Format("A {0} XML element is missing a required {1} child.",
                                                             "Node", "Nodes"));
            }
            DeserializeNodes(oXmlTextReader, oTreemapComponent, node.Nodes);
            if (!oXmlTextReader.Read() || oXmlTextReader.NodeType != XmlNodeType.EndElement ||
                oXmlTextReader.Name != "Node")
            {
